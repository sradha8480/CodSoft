 ...
 *
 * THIS STARTER CODE WORKS PARTIALLY BUT SOME METHODS REQUIRE COMPLETING - SEE
 * FURTHER DOWN IN THIS COMMENT
 *
 * The contact address book contains a 'database' of names, addresses and other
 * details
 ...
 * the information is built-in to the program (whereas in a 'serious' system it
 * would,
 ...
 * The core data processing methods for the other six buttons REQUIRE
 * COMPLETING.
 *
 ...
 * deleteContact, clearContacts, findContact, findPartial, sortAtoZ and
 * sortZtoA.
 *
...
public class addressbook extends JFrame implements ActionListener {
    ...
    private final Color veryLightGrey = new Color(240, 240, 240),
            darkBlue = new Color(0, 0, 150),
            backGroundColour = veryLightGrey,
            navigationBarColour = Color.lightGray,
            textColour = darkBlue;
    private static final int windowWidth = 450, windowHeight = 600, // Overall frame dimensions
            windowLocationX = 200, windowLocationY = 100; // and position
    private final int panelWidth = 450, panelHeight = 250, // The drawing panel dimensions
    ...
    private final Font mainHeadingFont = new Font("SansSerif", Font.BOLD, 20),
            detailsFont = new Font("SansSerif", Font.PLAIN, 14);

    /** The navigation buttons. */
    private JButton first = new JButton("|<"), // For "move to first contact" action
    ...
    /** The action buttons */
    private JButton addContact = new JButton("Add new contact"), // To request adding a new contact
            ...
            findPartial = new JButton("Find partial name"), // To find contact by partial, case insensitive match of
                                                            // name
    ...
    /**
     * Display non-background colour areas, heading and currently selected database
     * contact.
     *
    ...
    public void actionPerformed(ActionEvent e) {
        // If first is clicked: Cause the 0th contact to become selected (or -1 if there
        // are none)
        ...
        // If previous is clicked: Cause the previous contact to become selected, if
        // there is one
        if (e.getSource() == previous && currentContact > 0)
            currentContact--;

        // If next is clicked: Cause the next contact to become selected, if there is
        // one
        if (e.getSource() == next && currentContact < currentSize - 1)
            currentContact++;

        // If last is clicked: Cause the final available contact to become selected (or
        // -1 if there are none)
        if (e.getSource() == last)
     ...
     * If there is no following contact (that is, just deleted the highest indexed
     * contact),
     * then the previous becomes selected.
     * If there is no previous (that is, just deleted contact 0), then all contacts
     * have
    ...
    /**
     * Search for the contact whose name is an exact match to the name given in the
     * name text field.
     *
     * The search name must not be empty.
     * If found then the contact becomes selected.
     * If not found then the user is notified, and the selected contact does not
     * change.
     */
    ...
    /**
     * Search for the contact whose name contains the text given in the name text
     * field,
     * case insensitively.
     *
    ...
    /**
     * Re-order the contacts in the database so that the names are in ascending
     * alphabetic order
     *
    ...
    /**
     * Re-order the contacts in the database so that the names are in descending
     * alphabetic order
     *
     * The first contact becomes selected, provided that there is one.
     */
    ...
    private String[] name = new String[databaseSize],
     ...
     *
     * There is always one selected contact, unless there are no entries at all in
     * the database.
     * If there are one or more entries, then currentContact has a value in range 0
     * .. currentSize-1.
    ...
    /**
     * Add a new contact to the database in the next available location, if there is
     * space.
     *
     ...
     *
     * All contacts in subsequent (higher indexed) elements of the arrays are moved
     * "down" to fill the "gap".
     * The order of the remaining contacts is unchanged (for example, if previously
    ...
    /**
     * Search the database for a contact whose name contains the given search text,
     * case insensitively.
     *
     * Return the index of the match found, or -1 if no match found.
     */
